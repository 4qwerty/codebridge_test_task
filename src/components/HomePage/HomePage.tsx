...
const HomePage = () => {
  ...
  const filteredArticles = (articles: ArticleModel[], value: string) => {
    const filteredByTitle = articles.filter(({ title }) => filteredText(title, value))

    const filteredBySummary = articles.filter(({ summary }) => filteredText(summary, value))
  ...
  return (
    <Grid container direction='column' justifyContent='center' alignItems='center'>
      <SearchInput onChange={onChangeInput} value={inputValue} />
